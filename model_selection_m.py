...
from combiner_m import BanditCombiner
from basealg_OverOpt import BaseAlg
...
m_star = 2
alpha_star = -3
flag_rising = [0 if alpha_star < 0 else 1]
# size L
m_range = np.array([0, m_star, 3])  # range of values for m
alpha_range = np.array([alpha_star])  # np.array([-2, 0, 2])  # range of values for alpha
# horizon
T = 1200  # must be divisible by every (m + L) with m in m_range and L in L_range
...
L_range = np.array([1, 15, 20]) #2, 3, 4])
...
for run in range(n_runs):
    ...
    combiner = BanditCombiner(d, T, m_range, m_star, alpha_range, L_range, n_models, noise_level, delta_regret, lambda_A, alpha_regret)
    ...
    while t < T + (m_range[-1:] + L_range[-1:]):
        ...
        LBwM[idx_arm].update_beta(idx_arm)
        LBwM[idx_arm].est_theta_hat()

        #Update the Bandit Combiner
        combiner.update_stats_and_discard(idx_arm, avg_rwd)  # update also UCBs
    ...
    runs_rwds = np.append(runs_rwds, rwds[0:1200])
    print("t: ", t)
    ...
    torch.autograd.set_detect_anomaly(True)
    L_o = 5 # 3
    mab_o = MAC_Bandit(d, m_star, L_o, theta, alpha_star, lambda_A, noise_level, flag_rising)
    ...
    # for each time step
    #print("Tot rounds: ", T // (m_star + L_o))
    T_adj = int((1200 + (1200 - (T / (m_star + L_o)))))
    for tau in range(int((T_adj )// (m_star + L_o))): # ((1200 + (1200 - T / (m_star + L_o))) / (m_star + L_o)) * L_o  # ((T + 400) // (m_star + L_o)):
        #print("round: ", tau)
    ...
    runs_rwds_o = np.append(runs_rwds_o, rwds_o[0:1200])
...
#print("Total reward OverOpt: ", rwds_o.sum())
...
ax = plt.legend(labels=["Bandit Combiner", "OFUL"], fontsize=20)  # "OFUL-memory", "Bandit Combiner", "OFUL"], fontsize=20)
ax = plt.title("Model selection on m")
ax = plt.grid()

name_fig = "Performance_model_selection_m_LinUCB.pdf"
