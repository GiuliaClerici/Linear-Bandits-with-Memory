...
class BanditCombiner:
    ...
    def __init__(self, d, T, m_range, alpha_range, L_range, n_models, noise_level, delta_regret, lambda_A):
        ...
        self.alpha_regret = 0.5
        self.c_coeff_tot = np.ones((n_models))
        self.c_coeff = np.zeros((n_models))
        self.eta = np.ones((n_models)) * (np.sqrt(n_models) / np.sqrt(self.T))
    ...
    def putative_regret(self):
        ...
        for i in range(self.n_models):
            ...
            self.c_coeff[i] = 4 * self.L_range[idx_m] * op_norm * \
                              np.sqrt(self.d * np.log(1 + (self.t_plays[i] * op_norm**2 )/(self.d * self.lambda_A))) \
                              * (np.sqrt(self.lambda_A) + np.sqrt(np.log(1/self.delta_regret) + self.d * np.log(1 + (self.t_plays[i] * op_norm**2 )/(self.d * (self.m_range[idx_m] + self.L_range[idx_m]) * self.lambda_A))))
    ...
    def target_regret(self):
        for i in range(self.n_models):
            ...
            self.c_coeff_tot[i] = 4 * self.L_range[idx_m] * op_norm * \
                              np.sqrt(self.d * np.log(1 + (self.T * op_norm**2 )/(self.d * self.lambda_A))) \
                              * (np.sqrt(self.lambda_A) + np.sqrt(np.log(1/self.delta_regret) + self.d * np.log(1 + (self.T * op_norm**2 )/(self.d * (self.m_range[idx_m] + self.L_range[idx_m]) * self.lambda_A))))
            ...
            #               * ((1 + self.alpha_regret) ** (1 / self.alpha_regret)) )/( self.alpha_regret ** ((1 - self.alpha_regret)/self.alpha_regret))))
            self.tar_regs[i] = (self.c_coeff_tot[i] * (self.T ** self.alpha_regret)) + second_term + 288 * np.log((self.T**3) * self.n_models/self.delta_regret) * self.T * self.eta[i] + sum_etas
    ...
    def update_stats_and_discard(self, best_idx, rwd):
        ...
        # update UCB index
        self.u_inds[best_idx] = self.mean_rwds[best_idx] + min(1, ((self.put_regs[best_idx] + np.sqrt(
                                8 * np.log(((self.T ** 3) * self.n_models) / self.delta_regret) * self.t_plays[best_idx])) / self.t_plays[best_idx])) #- \
        ...
        thresh = self.put_regs[best_idx] + (3 * np.sqrt(np.log(((self.T**3) * self.n_models) / self.delta_regret)
